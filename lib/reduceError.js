...
const defaultErrorMessage = oneLine`
  There was an issue executing your request. Please try again later or contact
  support if you would like help resolving this issue.
`
const defaultRequestErrorMessage = oneLine`
...
export default function reduceError (err = {}) {
  ...
  if (request) {
    ...
    if (response.body?.feedback) {
      reduced.message = oneLine`
        There was an issue executing your request. Please correct any issues
        highlighted below.
      `
      reduced.feedback = response.body.feedback
    }
